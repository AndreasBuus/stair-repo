...
%   alternativ input (alignStep) - 'four_begin','second_end', 'second_begin'
    ...
    if strcmp(p.Results.alignStep, 'six_begin')
        ...
        for i = 1:size(FSR,1)
            stand_dur_index_aray(i) = step_index(i, fall) - step_index(i, rise); % [array(:)]
        end 
   
        for i = 1:size(FSR,1) % sweeps
            array1   = step_index(i,rise)-samplesBefore; % [index]
            arrayEnd = (step_index(i,rise) + floor(mean(stand_dur_index_aray))) + samplesAfter; % [index]
    
            stand_arr = [array1:arrayEnd]; % [index array]
        ...
        time_align = linspace(-p.Results.sec_before, (floor(mean(stand_dur_index_aray)) + samplesAfter)*dt, length(stand_arr));
    end
    ...
    if strcmp(p.Results.alignStep, 'four_begin')
        ...
        for i = 1:size(FSR,1)
            stand_DUR(i) = step_index(i, 4) - step_index(i, 5); % [array(:)]
        end 
   
        for i = 1:size(FSR,1) 
            array1   = step_index(i,5)-samplesBefore; % [index]
            arrayEnd = (step_index(i,5) + floor(mean(stand_DUR))) + samplesAfter; % [index]
        ...
        time_align = linspace(-p.Results.sec_before, (floor(mean(stand_DUR)) + samplesAfter)*dt, length(stand_arr));
    end
    ...
    if strcmp(p.Results.alignStep, 'second_begin')
        ...
        for i = 1:size(FSR,1)
            stand_DUR(i) = step_index(i, 6) - step_index(i, 7); % [array(:)]
        end 
    

        for i = 1:size(FSR,1) 
            array1   = step_index(i,7)-samplesBefore; % [index]
            arrayEnd = (step_index(i,7) + floor(mean(stand_DUR))) + samplesAfter; % [index]
